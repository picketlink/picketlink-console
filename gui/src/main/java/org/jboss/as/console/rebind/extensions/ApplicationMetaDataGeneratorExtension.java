...
import org.jboss.as.console.rebind.forms.FormItemDeclaration;
 ...
 * @sice Mar 14, 2012
 */
public class ApplicationMetaDataGeneratorExtension extends Generator {

    private static final String DEFAULT_BEAN_FACTORY_NAME = "org.jboss.as.console.client.shared.BeanFactory";
    /**
    ...
    private void generateFields(SourceWriter sourceWriter) {
        sourceWriter
                .println("private Map<Class<?>, List<PropertyBinding>> registry = new HashMap<Class<?>,List<PropertyBinding>>();");
        ...
        sourceWriter.println("private " + getBeanFactory() + " beanFactory = com.google.gwt.core.client.GWT.create("
                + getBeanFactory() + ".class);");
    }
    ...
    protected String getBeanFactory() {
        return DEFAULT_BEAN_FACTORY_NAME;
    }
    ...
    private void generateConstructor(TreeLogger logger, GeneratorContext context, SourceWriter sourceWriter) {
        ...
        /*
         * sourceWriter.println("String label = \"\";"); sourceWriter.println("Class<?> listType = null;");
         * sourceWriter.println("String subgroup = \"\";"); sourceWriter.println("String tabName = \"\";");
         * sourceWriter.println("String[] acceptedValues = null;");
         */

        try {
            Class<?> beanFactoryClass = getClass().getClassLoader().loadClass(getBeanFactory());
            ...
            throw new RuntimeException("Failed to load " + getBeanFactory());
        }
    ...
    private static FormItemDeclaration createFormItemDeclaration(Method method) {
        ...
        FormItemDeclaration decl = new FormItemDeclaration(defaultValue, label, localLabel, required, formItemTypeForEdit,
                formItemTypeForAdd, subgroup, tabName, order, acceptedValues);
        return decl;
    }
                ...
                if (returnType instanceof ParameterizedType) {
                    ...
                    if (typeArguments[0] instanceof Class) {
                        ...
                        for (PropBindingDeclarations binding : bindings) {
                            ...
                            sourceWriter.println("mut_" + idx + ".register(\"" + bindDecl.getJavaName() + "\", new Setter<"
                            ...
                            sourceWriter.println("mut_" + idx + ".register(\"" + bindDecl.getJavaName() + "\", new Getter<"
                                    + beanTypeClass.getName() + ">() {\n" + "public Object invoke("
                                    + bindDecl.getBeanClassName() + " entity) {\n" + "   return entity." + prefix
                                    + bindDecl.getPropertyName() + "();\n" + "}\n" + "});\n");
                        ...
                        // -----------------------------
                        // Factory lookup
                        sourceWriter.println("factories.put(" + beanTypeClass.getName() + ".class, new EntityFactory<"
